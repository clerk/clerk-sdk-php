...
> **Note:** If you use a `.env` file, you must load it in your PHP code. We recommend using [vlucas/phpdotenv](https://github.com/vlucas/phpdotenv):
>
> ```php
...
When custom error responses are specified for an operation, the SDK may also throw their associated exception. You can refer to respective _Errors_ tables in SDK docs for more details on possible exception types for each operation. For example, the `get` method throws the following exceptions:
